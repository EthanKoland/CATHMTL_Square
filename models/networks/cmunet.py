...
class MSAG(nn.Module):
    ...
    def __init__(self, channel):
        ...
        self.voteConv = nn.Sequential(
            nn.Conv2d(self.channel * 3, self.channel, kernel_size=(1, 1)), nn.BatchNorm2d(self.channel), nn.Sigmoid()
        )
...
class conv_block(nn.Module):
    ...
    def __init__(self, ch_in, ch_out):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(ch_in, ch_out, kernel_size=3, stride=1, padding=1, bias=True), nn.BatchNorm2d(ch_out),
            nn.ReLU(inplace=True), nn.Conv2d(ch_out, ch_out, kernel_size=3, stride=1, padding=1, bias=True),
...
class CMUNet(nn.Module):
    ...
    def __init__(self, img_ch=3, output_ch=1, l=7, k=7, multitask=False):
        """
        Args:
        ...
        if self.multitask:
            self.hm_conv = nn.Sequential(
                ...
                nn.Conv2d(in_channels=features, out_channels=output_ch, kernel_size=1)
            )
...
class CMUNetv2_CM(nn.Module):
    ...
    def __init__(
        self, img_ch=3, output_ch=1, dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 1, 1], k=[3, 3, 5, 7, 7], multitask=False
    ):
        """
        Args:
            img_ch : input channel.
