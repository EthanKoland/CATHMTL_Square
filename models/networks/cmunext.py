...
class fusion_conv(nn.Module):
    ...
    def __init__(self, ch_in, ch_out):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(ch_in, ch_in, kernel_size=3, stride=1, padding=1, groups=2, bias=True), nn.GELU(), nn.BatchNorm2d(ch_in),
            nn.Conv2d(ch_in, ch_out * 4, kernel_size=(1, 1)), nn.GELU(), nn.BatchNorm2d(ch_out * 4),
            nn.Conv2d(ch_out * 4, ch_out, kernel_size=(1, 1)), nn.GELU(), nn.BatchNorm2d(ch_out)
        )
    ...
    def __init__(
        ...
        if self.multitask:
            self.hm_conv = nn.Sequential(
                ...
                nn.Conv2d(in_channels=features, out_channels=num_classes, kernel_size=1)
            )
