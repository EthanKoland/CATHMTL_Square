...
class UNet(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=1, init_features=32, multitask=False):
    ...
    def forward(self, x):
        ...
        if self.multitask:
            output = {'mask': self.seg_conv(dec1), 'heatmap': self.hm_conv(dec1), 'blobsize': self.size_conv(dec1)}
        else:
            output = {'mask': self.seg_conv(dec1)}
